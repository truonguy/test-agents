...
## 9. Open Questions — trạng thái sau khi implement (T1–T12)

| # | Vấn đề | Trạng thái | Ghi chú |
|---|---|---|---|
| 1 | Tham số §7 (rate-limit, TTL, lockout, password policy) | ⏳ **CHỜ BA** | Đã code với giá trị đề xuất, đặt trong `config/auth_security.php` + `sanctum.inactivity_timeout` (override qua env) — **không hard-code**. Cần BA chốt con số trước merge. |
| 2 | Email driver thật cho reset | ⏳ **CHỜ HẠ TẦNG** | Test dùng `Mail::array`/`Notification::fake`. Link reset trỏ `FRONTEND_URL` (`ResetPassword::createUrlUsing`). Cấu hình SMTP/Mailgun + `FRONTEND_URL` khi deploy. |
| 3 | CORS/cookie domain 2 subdomain | ✅ **GIẢI QUYẾT (cấu hình)** | `config/cors.php` thêm: `paths=api/*`, origins qua `CORS_ALLOWED_ORIGINS`, `supports_credentials=false` (token-based). Domain thật điền qua env khi deploy. |
| 4 | i18n thông báo lỗi (VI/EN) | ⏳ **CHƯA LÀM** | Hiện message tiếng Anh cố định. Ngoài scope auth core; mở task riêng nếu cần. |
| 5 | Email verification cho customer mới | ✅ **CHỐT: KHÔNG** | Register → `ACTIVE` ngay, cấp token (FR-03). Nếu sau này cần, thêm `MustVerifyEmail`. |

---

## 10. Success Criteria — trạng thái

- [x] Toàn bộ AC-01..AC-07 pass bằng feature test (**70 tests / 194 assertions xanh**).
- [x] Cross-access Shop⇄CRM đều 401/403.
- [x] RBAC: employee không chạm manage_employee/system_config.
- [x] Security: rate-limit, audit, logout-all, lockout, TTL verify được.
- [x] Không endpoint nào lộ user-enumeration (`NoUserEnumerationTest`).
- [~] Coverage feature test auth ≥ 80%: **không đo tự động được** — môi trường thiếu coverage driver (Xdebug/PCOV). Định tính: mọi controller/service/endpoint auth đều có feature test (10 file test). Cần cài driver để xuất số chính thức.
- [x] Open Questions §9 cập nhật trạng thái; còn #1 (BA chốt tham số) & #2 (email driver) chặn merge/deploy.
